...
type Row = {
  ...
  yts_url: string | null;
  has_bluray_upgrade: number | null;
  bluray_qualities: string | null;
  ...
  const movies = db
    .prepare(
      `SELECT
         ...
         c.found AS yts_found, c.yts_url, c.has_bluray_upgrade, c.bluray_qualities,
         c.best_source, c.best_quality, c.checked_at
       FROM movies m
       LEFT JOIN yts_checks c ON c.movie_id = m.id
       ORDER BY
         CASE WHEN c.has_bluray_upgrade = 1 THEN 0 ELSE 1 END,
         m.title COLLATE NOCASE`
    )
    .all() as Row[];

  const upgradeCount = movies.filter((m) => m.has_bluray_upgrade === 1).length;
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500">
            ...
            <span className={upgradeCount > 0 ? 'text-emerald-600 dark:text-emerald-400 font-medium' : ''}>
              {upgradeCount} BluRay {upgradeCount === 1 ? 'upgrade' : 'upgrades'} available
            </span>
          </p>
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="text-left text-zinc-500 border-b border-zinc-200 dark:border-zinc-800">
                <tr>
                  ...
                  <th className="py-2 pr-4 font-medium">YTS Upgrade</th>
...
function UpgradeCell({ row }: { row: Row }) {
  ...
  if (row.has_bluray_upgrade === 1 && row.yts_url) {
    return (
      <a
        href={row.yts_url}
        target="_blank"
        ...
        title={`Available BluRay qualities: ${row.bluray_qualities ?? '—'}`}
      >
        BluRay {row.bluray_qualities ? `(${row.bluray_qualities})` : ''}
      </a>
    );
  }
  return (
    <span className="text-zinc-400 text-xs" title={`Best on YTS: ${row.best_source ?? '—'} ${row.best_quality ?? ''}`}>
      no BluRay
    </span>
  );
}
